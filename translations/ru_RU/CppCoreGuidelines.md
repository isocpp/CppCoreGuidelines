...
Этот документ представляет собой черновую копию. Он невреен, нпеолон и пÂµÃлохо отформатирован.
Это проект с открытым кодом, текущая версия 0.6.
Копирование, изменение, создание производных трудов и любое другое использование этого проекта регулируется лицензией MIT.
Внесение вклада в этот проект требует согласия с лицензией Contributor License. Для получения большей информации, см. сопутствующий файл [LICENSE](LICENSE).
Мы разрешаем использовать, копировать, изменять и использовать этот проект в качестве источника "дружественно настроенным пользователям", надеясь, что он станет источником конструктивной информации.
...
Комментарии и предложения по улучшению очень горячо приветствуются.
Мы планируем изменять и дополнять этот документ по мере роста нашего понимания, улучшения языка и набора доступных библиотек.
Перед тем как оставить комментарий, пожалуйста, посмотрите раздел [введение](#S-introduction), который отображает наши цели и общий подход.
Список участников [здесь](#SS-ack)*.
...
* Список правил пока не был целиком проверен на полноту, консистентность и возмжоность обеспечения применения.
* Тройной знак вопроса (???) означает, что в этом месте осутствует часть информации
* Обновить секцию ссылок; многие источники являются очень старыми (ранее С++11)
* Для более или менее актуального списка задач см: [To-do: Unclassified proto-rules](#S-unclassified)
